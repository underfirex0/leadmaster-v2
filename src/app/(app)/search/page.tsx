...
const SIZES = [10,25,50,100,500,1000,5000,10000]
              ...
              {sector.domaines.length > 1 && (() => {
                ...
                return (
                  <div onClick={() => { if (domState === 'all') domSlugs.forEach(s => onToggle(s,false)); else domSlugs.forEach(s => onToggle(s,true)) }}
                    className={cn('flex items-center gap-2 px-3 py-1.5 rounded-xl cursor-pointer text-[12px] font-bold uppercase tracking-wide transition-colors mt-1',
                      domState !== 'none' ? 'text-indigo-700 bg-indigo-50' : 'text-gray-400 hover:bg-gray-50')}>
                    <CheckIcon state={domState} sm />
                    {dom.domaine}
                    <span className="ml-auto text-[10px] font-normal text-gray-400">{domSlugs.length}</span>
                  </div>
                )
              })()}
...
export default function SearchPage() {
  ...
  const [estimate, setEstimate] = useState<{ count: number; cost: number; freeTrialEligible?: boolean; coverage?: Record<string,number> } | null>(null)
  ...
  useEffect(() => {
    if (!selected.size && !cities.length && !nameSearch) { setLiveCount(null); return }
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const r = await fetch('/api/search/estimate', {
          ...
          body: JSON.stringify({ activites:[...selected], cities, name:nameSearch, fields:[...selectedFields,'basic'], limit:maxCompanies }),
        })
        const d = await r.json()
        setLiveCount(d.count??0)
        setEstimate(d.count !== undefined ? { count:d.count, cost:d.estimatedCost, freeTrialEligible:d.freeTrialEligible, coverage:d.fieldCoverage } : null)
        setBalance(d.balance)
      } finally { setLiveLoading(false) }
    }, 600)
  }, [selected, cities, nameSearch, selectedFields, maxCompanies])
  ...
  function toggleField(fid:string){setSelectedFields(prev=>{const n=new Set(prev);n.has(fid)?n.delete(fid):n.add(fid);return n})}
  function applyPreset(fields:string[]){setSelectedFields(new Set(fields))}

  // Cost = basic (1cr, always) + selected extra fields
  const costPerBiz = useMemo(()=>{
    const base = FIELD_GROUPS.basic.cost // 1cr
  ...
  async function handleLaunch() {
    if (!hasFilter && !isFreeTrial) { showToast('Ajoutez au moins un filtre','error'); return }
    setLaunching(true)
    try {
      ...
      if (d.freeTrialUsed) showToast('🎁 Essai gratuit utilisé ! 100 entreprises déverrouillées','info')
      if (d.creditsSpent>0) setBalance(d.newBalance)
      router.push('/databases')
    } catch { showToast('Erreur réseau','error') } finally { setLaunching(false) }
  }
    ...
    <div className="min-h-screen bg-gray-50">
      {toast && (
        <div className={cn('fixed top-4 right-4 z-50 px-4 py-3 rounded-2xl shadow-xl text-[13.5px] font-semibold text-white max-w-sm',
          toast.type==='success'?'bg-emerald-600':toast.type==='error'?'bg-red-500':'bg-indigo-600')}>
          {toast.msg}
        </div>
      )}
        ...
        {freeTrialAvail && (
          <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 mb-6 flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-emerald-600 text-[13px]">Obtenez 100 entreprises gratuitement avec le profil de base (nom, ville, secteur, activités, forme juridique, réseaux sociaux). Ne sélectionnez aucun champ supplémentaire et choisissez max 100 entreprises.</div>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-6">
                ...
                <div>
                  ...
                  <div className="relative">
                    <button onClick={()=>setCityOpen(!cityOpen)}
                      className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-[13px] text-left flex items-center justify-between focus:outline-none focus:border-indigo-400">
                        ...
                        <div className="max-h-48 overflow-y-auto p-1.5">
                          {allCities.map(c=>(
                            <label key={c} className="flex items-center gap-2.5 px-3 py-1.5 rounded-xl hover:bg-gray-50 cursor-pointer text-[13px]">
                              <input type="checkbox" checked={cities.includes(c)} onChange={()=>{setCities(prev=>prev.includes(c)?prev.filter(x=>x!==c):[...prev,c])}} className="accent-indigo-600"/>
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="font-semibold text-[14px] text-gray-800 flex items-center gap-2">
                    ...
                    {liveCount !== null && !liveLoading && (
                      <span className="text-[12px] font-normal text-indigo-600 bg-indigo-50 rounded-full px-2 py-0.5">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="font-bold text-[13px] text-indigo-800 flex items-center gap-2">
                      🏢 {FIELD_GROUPS.basic.label}
                      <span className="text-[11px] font-semibold text-indigo-600 bg-white border border-indigo-200 rounded-full px-2 py-0.5">INCLUS</span>
                    </div>
                    <div className="text-[12px] text-indigo-600 mt-0.5">{FIELD_GROUPS.basic.description}</div>
                  </div>
                  <span className="font-bold text-indigo-700 text-[15px]">{FIELD_GROUPS.basic.cost} cr</span>
                </div>
              </div>

              {/* Free row always included */}
              <div className="mb-3 p-2.5 bg-emerald-50 rounded-xl border border-emerald-100 text-[12px] text-emerald-700">
                <span className="font-semibold">Toujours visible gratuitement (aperçu) :</span> Nom d'entreprise · Ville
              </div>
          ...
          <div className="space-y-4 lg:sticky lg:top-20 lg:self-start">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="text-[48px] font-bold text-gray-900 font-mono tabular-nums leading-none">{maxCompanies.toLocaleString('fr-FR')}</div>
              ...
              <div className="flex gap-1.5 flex-wrap">
                {SIZES.map(s=>(
                  <button key={s} onClick={()=>setMaxCompanies(s)}
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              {/* Smart cost note */}
              {estimate && estimate.coverage && (
                <div className="mt-3 p-3 bg-amber-50 border border-amber-100 rounded-xl">
                  <div className="text-[11.5px] text-amber-700 font-semibold mb-1">💡 Coût réel sera inférieur</div>
                  <div className="text-[11px] text-amber-600">Le coût est calculé uniquement pour les entreprises qui ont chaque donnée. Les champs manquants ne sont pas facturés.</div>
                  <div className="text-[13px] font-bold text-amber-800 mt-1">
                    ≈ {estimate.cost.toLocaleString('fr-FR')} cr estimés
                  </div>
                </div>
              )}
              ...
              {balance !== null && !isFreeTrial && balance < maxEstCost && maxEstCost > 0 && (
                <div className="mt-3 flex items-start gap-2 p-3 bg-red-50 border border-red-100 rounded-xl">
                  <AlertCircle className="w-4 h-4 text-red-500 shrink-0 mt-0.5"/>
                  <span className="text-[12px] text-red-600">Solde peut être insuffisant. Le coût réel sera calculé précisément avant déduction.</span>
                </div>
              )}
            </div>
            ...
            <button onClick={handleLaunch} disabled={launching}
              ...
              {launching
                ...
                : isFreeTrial
                  ? <>🎁 Obtenir mes 100 entreprises gratuitement</>
                  : <>Lancer la recherche →</>
              }
            </button>
            ...
            <div className="bg-gray-50 border border-gray-100 rounded-xl p-4 text-[12px] text-gray-500 space-y-1.5">
              ...
              <div className="flex gap-1.5"><span className="text-emerald-500 shrink-0">✓</span> Aperçu gratuit : nom + ville de toutes les entreprises</div>
              <div className="flex gap-1.5"><span className="text-indigo-500 shrink-0">✓</span> 1 cr minimum pour le profil de base (6 champs)</div>
              <div className="flex gap-1.5"><span className="text-indigo-500 shrink-0">✓</span> Coût calculé uniquement si la donnée existe</div>
              <div className="flex gap-1.5"><span className="text-amber-500 shrink-0">✓</span> Données prioritisées : d'abord les entreprises les plus complètes</div>
              <div className="flex gap-1.5"><span className="text-purple-500 shrink-0">✓</span> Déblocage progressif : ajoutez des champs après coup</div>
            </div>
          </div>
        </div>
      </div>
