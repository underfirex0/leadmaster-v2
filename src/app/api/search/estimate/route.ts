...
async function fetchAllMatching(
  ...
  // RPC-based filtering — the sector/domaine/activité arrays travel in the
  // POST body instead of a URL query string, so there's no risk of the
  // request silently breaking once many items are selected (previously a
  // giant .or()/.in() filter string built into the URL could exceed
  // proxy/edge URL-length limits and return 0 rows with zero error).
  const rpcArgs = {
    p_sectors:   filters.sectors,
    p_domaines:  filters.domaines,
    p_activites: filters.activites,
  ...
  for (let offset = 0; offset < fetchTarget; offset += BATCH) {
    const { data: batch, error } = await supabaseAdmin.rpc('fetch_companies_matching', {
      ...rpcArgs, p_limit: BATCH, p_offset: offset,
    })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: queryRecord, error: qErr } = await supabaseAdmin
      .from('queries').insert({
        user_id:          user.id,
        filters:          { sectors, domaines, activites, cities, name, capital_min, capital_max, effectif: effectif||undefined, effectifs: effectifs?.length>0 ? effectifs : undefined },
